...
#import "num/num.typ"
#import "angle.typ": (
  is-angle, parse as parse-angle, to-number as angle-to-number,
)
...
#let parse(options, real, imag) = {
  ...
  let mode = if options.complex-mode == "input" { input-mode } else {
  ...
  return (
    ...
    if options.complex-mode == "input" and input-mode == "cartesian"
      or options.complex-mode == "cartesian" {
...
#let complex(real, imag, unit, options) = {
  ...
  let real-mantissa = {
    let (options, sign, mantissa, ..rest) = num.process(
      options,
      ..real,
      none,
      none,
