...
#meander.reflow({
    ...
    placed(top + left, {
      ...
      show: figure.with(caption: [@src_bloodsmear_macro Blutausstriche auf einer Mikroskopplatte; Unterscheidung zwischen dünnen und dickem Film.])
      show: rect.with(inset: 0pt)
      show: scale.with(x: -100%)
      image("assets/bloodsmear_macro.jpg")
    }, boundary: contour.margin(5mm))

    placed(horizon + right, {
      show: box.with(width: 50%)
      show: it => [#it <bloodsmear-microscopy>]
      show: figure.with(caption: [@src_bloodsmear_microscopy Mikroskopische Untersuchung eines Blutausstriches. Erkennbar sind die Erythrozyten und Leukozyten.])
      show: rect.with(inset: 0pt)
      show: scale.with(x: -100%)
      image("assets/bloodsmear_micro.jpg")
    }, boundary: contour.margin(5mm), dy: -1cm)
