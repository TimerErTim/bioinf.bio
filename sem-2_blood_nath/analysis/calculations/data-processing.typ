#import "../../../lib/maths/statistics.typ": chi-square-ablehnungsbereich, chi-square-teststatistic, mean, stddev, median
...
#let _observed-to-reference-lable-map = (
  "Neutrophile": (
    adult-leukozyten-reference-data.at("Segmentkernige neutrophile Granulozyten"),
    adult-leukozyten-reference-data.at("Stabkernige neutrophile Granulozyten"),
  ),
  "Eosinophile": (adult-leukozyten-reference-data.at("Eosinophile Granulozyten"),),
...
#let adult-leukozyten-reference-data-observed-format = {
  let unnormalized = _observed-to-reference-lable-map
    .pairs()
    .map(((key, values)) => (key, values.map(it => it.at("relativer_anteil_prozent").sum() / 2).sum()))
...
#let chi-square-teststatistic-adult-leukozyten(entries-observed) = {
  ...
  let observed-counts = reference-probs.pairs().map(((key, _)) => entries-observed.at(key))
  let total-observed-count = observed-counts.sum()
  let expected-counts = reference-probs.pairs().map(((key, value)) => value * total-observed-count)
  chi-square-teststatistic(expected-counts, observed-counts)
}

/// Entries observed shape: ("<Type>": absolute-count, ...)
#let is-difference-adult-leukozyten-significant(entries-observed, alpha: 0.05) = {
  (
    chi-square-teststatistic-adult-leukozyten(entries-observed).t
      > chi-square-ablehnungsbereich(df: entries-observed.len() - 1, alpha: alpha)
  )
}


#let data = csv("../data/UE_Blut_Ergebnisse.xlsx - Blutausstrich.tsv", delimiter: "\t", row-type: array).map(
  it => it.slice(0, 10),
)
...
#let convert-entry-to-observed(entry) = {
  let parsed-entry = ("Neutrophile", "Eosinophile", "Basophile", "Monozyten", "Lymphozyten")
    .map(key => {
...
#let data-dict = data-dict.map(entry => {
  let entry-has-allergy = (
    entry.Allergie.trim() != "" and entry.Allergie.trim() != "n" and entry.Allergie.trim() != "nein"
  )
  ...
  let entry = (..entry, "has-allergy": entry-has-allergy, "has-acute-erkrankung": entry-has-acute-erkrankung)
  ...
  (
    ...
    chi-square-deviations: _observed-to-reference-lable-map
      ...
      .to-dict(),
    difference-significant: is-difference-adult-leukozyten-significant(observation),
  )
})

#let data-all-group = data-dict.filter(it => it.at("unprocessable", default: false) == false)
#let data-current-year-group = data-all-group.filter(it => it.Stdgang == "MBI25")
#let data-allergies-group = data-all-group.filter(it => it.has-allergy)
#let data-acute-erkrankungen-group = data-all-group.filter(it => it.has-acute-erkrankung)
...
#let stats-acute-erkrankungen-group = stats-for-set(data-acute-erkrankungen-group)
