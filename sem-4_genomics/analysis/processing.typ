...
#let liver-weights-data = (
  liver-weights-raw-data.map(it => (it.initials, it)).to-dict()
)

#let restr-enzyme-distr-raw-data = json("../data/restr_enzyme_distr.json")
#let restr-enzyme-distr-data = (
  restr-enzyme-distr-raw-data.map(it => (it.initials, it)).to-dict()
)
