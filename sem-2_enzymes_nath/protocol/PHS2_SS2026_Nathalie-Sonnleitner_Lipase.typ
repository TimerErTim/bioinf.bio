...
#figure(
  average-ph-drop-plot(width: 80%, height: 6cm),
...
#figure(
  grid(
    rows: (auto, auto),
    gutter: 1em,
    average-ph-drop-by-amount-uncooked-plot(width: 90%, height: 6cm),
    average-ph-drop-by-amount-cooked-plot(width: 90%, height: 6cm),
  ),
...
#figure(
  average-ph-drop-plot(width: 80%, years: (2026,), title: [pH-Verlauf für MBI25], height: 6cm),
  caption: [Example of a pH vs. time plot for a single experimental group (MBI25, Group 1A).],
) <fig-group-example>
...
1. *Paired T-Test (Active vs. Inactive Lipase):*
  A paired t-test was used to compare the pH drop between the unboiled and boiled samples from the same experimental run. The test yielded a _p-value of #str(paired-t-tests-active-vs-inactive.p-value)_, which is far below the standard significance level of 0.05. This result is *highly statistically significant* and confirms that the active, unboiled lipase produces a much greater pH drop than the denatured, boiled lipase.

2. *ANOVA (Effect of Enzyme Amount):*
  An ANOVA test was conducted to determine if the amount of lipase added (from 50 #sym.mu\L to 600 #sym.mu\L) had a statistically significant effect on the magnitude of the pH drop. The p-value from this test was _#str(anova-tests-amount.p-value)_. Since this value is greater than 0.05, we conclude that there is *no statistically significant difference* in the pH drop between the different amounts of enzyme used in this set of experiments. While a trend is visible in the data and plots, the high variability in the data prevented this trend from being statistically significant.
